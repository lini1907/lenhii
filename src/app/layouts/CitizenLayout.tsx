...
import { ShieldAlert, Home, AlertTriangle, User, LogOut, Bell, Menu, X, List } from "lucide-react"
...
export function CitizenLayout() {
  ...
  useEffect(() => {
    getCurrentUserWithProfile().then((result) => {
      if (!result || result.profile.role !== "citizen") {
  ...
  const navItems = [
    ...
    { name: "Submit SOS", path: "/citizen/sos", icon: ShieldAlert },
    { name: "My Requests", path: "/citizen/my-requests", icon: List },
  ]
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-900">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 transition-transform duration-300 ease-in-out md:static md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between h-16 px-4 bg-slate-950">
          <Link to="/citizen" className="flex items-center gap-2 text-white">
            <div className="w-8 h-8 bg-red-600 rounded flex items-center justify-center">
              <ShieldAlert className="w-5 h-5 text-white" />
            </div>
            <span className="font-bold text-lg tracking-tight">Citizen Portal</span>
          </Link>
        ...
        <nav className="p-4 space-y-1 overflow-y-auto h-[calc(100vh-8rem)]">
          <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4 mt-4">Menu</div>
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
        ...
        <div className="p-4 border-t border-slate-800 absolute bottom-0 w-full">
          <button
            type="button"
            onClick={() => {
              signOut().finally(() => navigate("/auth/citizen/login", { replace: true }))
            }}
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:bg-red-900/50 hover:text-red-400 w-full text-left"
          >
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 z-10">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold text-slate-900 capitalize tracking-tight hidden sm:block">
              {location.pathname === "/citizen" ? "Dashboard" : location.pathname.split('/').pop()?.replace('-', ' ')}
            </h1>
          </div>
